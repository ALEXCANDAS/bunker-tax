...
# Inicializamos el estado de sesión para que NADA esté vacío al cargar
...
def recalcular():
    ...
    # Lógica de Inversión del Sujeto Pasivo (ISP)
...
# 3. INTERFAZ: PESTAÑAS (DECLARADAS ANTES DE USARLAS)
tab_rec, tab_emi, tab_ctrl = st.tabs(["📥 RECIBIDAS", "📤 EMITIDAS", "📋 CONTROL"])
...
with tab_rec:
    ...
    with col_pdf:
        ...
        # Cuadro de Asiento Contable previo para auditar el ISP
        st.markdown(f"""
    ...
    with col_ficha:
        # FORMULARIO MAESTRO (PARA CAPTURAR EL ENTER)
        with st.form("form_contable", clear_on_submit=False):
